    ...
    describe CombinatorialGeneratedExpressionFactory do
      it "generates multiple expressions" do
        parameter_type_combinations = [
          [
            ParameterType.new("color", /red|blue|yellow/, Color, lambda { |_s| Color.new }, true, false),
            ParameterType.new("csscolor", /red|blue|yellow/, CssColor, lambda { |_s| CssColor.new }, true, false)
          ],
          [
            ParameterType.new("date", /\d{4}-\d{2}-\d{2}/, Date, lambda { |_s| Date.new }, true, false),
            ParameterType.new("datetime", /\d{4}-\d{2}-\d{2}/, DateTime, lambda { |_s| DateTime.new }, true, false),
            ParameterType.new("timestamp", /\d{4}-\d{2}-\d{2}/, Timestamp, lambda { |_s| Timestamp.new }, true, false)
          ]
        ]
