  ...
  module CucumberExpressions
    ...
    describe ParameterTypeRegistry do
      ...
      it "does not allow more than one prefer_for_regexp_match parameter type for each regexp" do
        @registry.define_parameter_type(ParameterType.new("name", CAPITALISED_WORD, Name, lambda { |_s| Name.new }, true, true))
        @registry.define_parameter_type(ParameterType.new("person", CAPITALISED_WORD, Person, lambda { |_s| Person.new }, true, false))
        expect do
          @registry.define_parameter_type(ParameterType.new("place", CAPITALISED_WORD, Place, lambda { |_s| Place.new }, true, true))
        end.to raise_error(
      ...
      it "looks up prefer_for_regexp_match parameter type by regexp" do
        name = ParameterType.new("name", CAPITALISED_WORD, Name, lambda { |_s| Name.new }, true, false)
        person = ParameterType.new("person", CAPITALISED_WORD, Person, lambda { |_s| Person.new }, true, true)
        place = ParameterType.new("place", CAPITALISED_WORD, Place, lambda { |_s| Place.new }, true, false)
      ...
      it "throws ambiguous exception when no parameter types are prefer_for_regexp_match" do
        name = ParameterType.new("name", CAPITALISED_WORD, Name, lambda { |_s| Name.new }, true, false)
        person = ParameterType.new("person", CAPITALISED_WORD, Person, lambda { |_s| Person.new }, true, false)
        place = ParameterType.new("place", CAPITALISED_WORD, Place, lambda { |_s| Place.new }, true, false)
